...
# ═════════════════════ CLI front-end (--force) ════════════════════════════
def _cli():
    ap = argparse.ArgumentParser(description="GPU Activation-DB utility")
    sub = ap.add_subparsers(dest="cmd", required=True)

    b = sub.add_parser("build")
    b.add_argument("--model", required=True)
    b.add_argument("--hooks", required=True)
    b.add_argument("--dataset", nargs=2,
                   default=("wikitext", "wikitext-103-raw-v1"))
    b.add_argument("--tokens", type=int, default=200_000)
    b.add_argument("--batch", type=int, default=256)
    b.add_argument("--seq",   type=int, default=1024)
    b.add_argument("--force", action="store_true")
    b.add_argument("--random", action="store_true")
    b.add_argument("--quiet",  action="store_true")

    l = sub.add_parser("list");  l.add_argument("--run", required=True)

    a = sub.add_parser("add")
    a.add_argument("--run", required=True)
    a.add_argument("--name", required=True)
    a.add_argument("--expr", required=True)
    a.add_argument("--hook")
    ...
    w.add_argument("--radius", type=int, default=8)
    ...
    if args.cmd == "build":
        ActivationDB.build(run_dir=None,
                           model=args.model,
                           hooks=[h.strip() for h in args.hooks.split(",") if h],
        ...
        db = ActivationDB(args.run, verbose=verbose)
        for name, toks in db._tag_tokens.items():
            print(f"• {name:<20}{toks.numel():>10,}")

    elif args.cmd == "add":
        db   = ActivationDB(args.run, verbose=verbose)
        code = compile(args.expr, "<expr>", "eval")
        db.add_tag(args.name,
                   lambda d, _c=code: eval(_c, {"np": np}, {"d": d}),
                   hook=args.hook)

    elif args.cmd == "windows":
        db = ActivationDB(args.run, verbose=verbose)
        outs = db.random_windows(args.tag, args.k, args.radius)
        if outs:
            for s, _ in outs: print("•", s)
        else:
            print(f"(no token satisfies tag '{args.tag}')")
