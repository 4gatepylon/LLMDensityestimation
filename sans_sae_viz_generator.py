...
def generate(root_folder: str, use_s3: bool, output_dir: str):
    compiler = WebsiteCompiler(root_folder, use_s3, output_dir)
    compiler.compile()

@cli.command()
@click.option("--folder-name", "-f", "-i", type=str, help="The folder name to deploy to S3")
def deploy(folder_name: str):
    """
    Upload a folder to s3://gpt2.pro/<folder_name> and index.html to s3://gpt2.pro/index.html
    if the folder doesn't already exist in S3. The folder must contain an index.html file.
    ...
    if not (index_path.exists() and index_path.is_file()):
        raise ValueError(f"{folder_name}/index.html does not exist.")
    subfolders = [f for f in folder_path.iterdir() if f.is_dir()]
    assert len(subfolders) == 1, f"Expected exactly one subfolder in {folder_name}, got {len(subfolders)}" # fmt: skip
    images_subfolder_path = subfolders[0]
    if not (images_subfolder_path.exists() and images_subfolder_path.is_dir()):
        raise ValueError(f"{folder_name}/images does not exist.")
    images_subfolder_name = images_subfolder_path.name # This has to be unique on S3 (so you must delete
    click.echo(f"Image subfolder name: {images_subfolder_name}")
    # the old one or... yea idk just make a new time for each new version)
    
    # Initialize S3 client
    try:
        s3 = boto3.client('s3')
        
        # Check permissions by listing the bucket
        try:
            s3.head_bucket(Bucket='gpt2.pro')
        except ClientError as e:
            error_code = e.response['Error']['Code']
            if error_code == '403':
                click.echo("Error: No permission to access the S3 bucket gpt2.pro")
            elif error_code == '404':
                click.echo("Error: The S3 bucket gpt2.pro does not exist")
            else:
        ...
        try:
            # List objects with the folder name prefix to see if anything exists
            response = s3.list_objects_v2(
                Bucket='gpt2.pro',
                Prefix=f"{images_subfolder_name}/",
                MaxKeys=1
            )
            
            if 'Contents' in response and len(response['Contents']) > 0:
                click.echo(f"Error: s3://gpt2.pro/{images_subfolder_name}/ already exists. Aborting.")
                sys.exit(1)
                
        except ClientError as e:
            click.echo(f"Error checking if folder exists: {e}")
            sys.exit(1)
        
        # Upload the entire folder
        click.echo(f"Uploading folder {images_subfolder_name} to s3://gpt2.pro/{images_subfolder_name}/")
        subfiles = list(images_subfolder_path.glob('**/*'))
        for file_path in tqdm.tqdm(subfiles, desc="Uploading images (and other files)"):
            if file_path.is_file():
                # Calculate the relative path from the folder being uploaded
                relative_path = file_path.relative_to(images_subfolder_path)
                s3_path = f"{images_subfolder_name}/{relative_path.as_posix()}"
                
                click.echo(f"Uploading {file_path} to s3://gpt2.pro/{s3_path}")
                s3.upload_file(str(file_path), 'gpt2.pro', s3_path)
        
        # Upload index.html to root (overwriting if necessary)
        click.echo(f"Uploading {index_path} to s3://gpt2.pro/index.html")
        s3.upload_file(index_path.as_posix(), 'gpt2.pro', 'index.html') # Overwrite?
