...
"""
A really dummy/simplistic set of methods to be able
to load and save safetensors activations that
may be very large (across multiple different files).
"""
...
def load_files2tensor(subfolder: Path) -> torch.Tensor:
    ...
    # & return
    assert (og_dim is None and meta.total_num_datapoints == 0) or (
        og_dim is not None and catted.ndim == og_dim
    )
    catted = catted.detach().requires_grad_(False).cpu()
    return catted


def store_tensor2files(
    tensor: torch.Tensor, output_subdir: Path, file_batch_size: int
) -> None:
    assert output_subdir.exists()
    assert output_subdir.is_dir()
    assert next(output_subdir.iterdir(), None) is None

    tensor = tensor.detach().requires_grad_(False).cpu()
