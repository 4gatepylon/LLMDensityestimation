...
def load_checkpoint(checkpoint_path: str | Path, infer_hyperparameters: Callable[[Any], dict]):
    ...
    hyperparameters = (
        json.loads(Path(hyperparameters_path).read_text()) if Path(hyperparameters_path).exists() else None
    )
...
def save_checkpoint(checkpoint_path: str | Path, hyperparameters: Optional[dict], state_dict: dict, clobber: bool = True):
    ...
    hyperparameters_path = checkpoint_path + "_hyperparameters.json"
    if not clobber:
        assert not Path(safetensors_path).exists(), "safetensors file already exists"
        assert not Path(hyperparameters_path).exists(), "hyperparameters file already exists"
